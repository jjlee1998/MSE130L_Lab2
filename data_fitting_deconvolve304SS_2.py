...
hcl_cat_df = dfs[10]
hcl_ano_df = dfs[11].iloc[23:,:]
split_pt = np.argmax(np.diff(hcl_ano_df['potential_V'].values) < 0)
hcl_ano_df_u = hcl_ano_df.iloc[:split_pt, :].copy()
hcl_ano_df_d = hcl_ano_df.iloc[split_pt:, :].copy()
split_phi = hcl_ano_df['potential_V'].iloc[split_pt]
hcl_ano_df_d['potential_V'] = 2*split_phi - hcl_ano_df_d['potential_V']
hcl_df = hcl_cat_df[::-1].append([hcl_ano_df_u, hcl_ano_df_d]).reset_index(drop=True)
...
ax.scatter(hcl_df['potential_V'], np.abs(hcl_df['j_mA/mm2']), s=0.5)
